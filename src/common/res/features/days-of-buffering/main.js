...
function ynabEnhancedDoBCalculate() {
    ...
    if (ynabToolKit.settings.daysOfBufferingHistoryLookup > 0) {
        dateNow = Date.now();
        var outflowTransactions = outflowTransactions.filter((el) => 
            (dateNow - el.getDate().getUTCTime()) / 3600/24/1000/(365/12) < ynabToolKit.settings.daysOfBufferingHistoryLookup)
    }
